...
jaxc_mapped_reader_t* mapped_reader_create(struct json_object* obj)
{
	jaxc_node_t* temp_node;
	jaxc_mapped_reader_t* reader;
	reader = (jaxc_mapped_reader_t*)malloc(sizeof(struct jaxc_mapped_reader_s));
...
void mapped_reader_free(jaxc_mapped_reader_t* reader)
{
	/*FIX->Axis free */
...
{	
	...
	{	
		mapped_reader_process_attributes(reader);
		printf ("Processing Start ELEMENT---------\n");
		...
		{	
			...
			{
				printf ("JSON _OBJ TEST: %s\n", json_object_to_json_string(reader->current_json_object));
			...
			else if(json_object_is_type(temp_json_obj, json_type_array))
			{
				...
				jaxc_node_set_current_element( jaxc_stack_get(reader->node_stack),
											"END_ARRAY_ELEMENT");
				mapped_reader_read(reader);	/* Recursion ... Critical  */
	...
	{
		printf ("Processing END_ELEMENT Event\n");
		jaxc_stack_pop(reader->node_stack);
			...
			if (temp_lhtable)
			{
				if (json_object_get_object(temp_lhtable->head->v))
				{
					...
					if (temp_lhtable->head)
					{
						...
						else printf ("No NEXTT : %d\n", jaxc_stack_size(reader->node_stack));
					}
				}
			}
			else 
	...
	{
		printf ("Processing CHARACTER Event\n");
		reader->jaxc_event = JAXC_END_ELEMENT;	
	}
	else
